...
async function yt(url, quality, type, bitrate, server = 'en68') {
    ...
    switch (type) {
        ...
        case 'mp3':
            list = {
                '128kbps': ([...table.querySelectorAll('td > a[href="#"]')].filter(v => !/\.3gp/.test(v.innerHTML)).map(v => [v.innerHTML.match(/.*?(?=\()/)[0].trim(), v.parentElement.nextSibling.nextSibling.innerHTML]))
            }
            break
        default:
            list = {}
